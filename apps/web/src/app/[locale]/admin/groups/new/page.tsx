  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="space-y-2">
        <Link href={`/${params.locale}/admin/groups`} className="text-sm text-gray-500 hover:text-gray-800">
          ← {zh ? '返回群組列表' : 'Back to groups'}
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">{zh ? '建立群組' : 'Create Group'}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">{zh ? '群組名稱' : 'Group name'}</label>
          <input
            value={name}
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">{zh ? '群組 PID' : 'Group PID'}</label>
          <input
            value={suggestedPid}
            readOnly
        ...
        <div className="space-y-3 rounded-xl bg-gray-50 p-4">
          ...
          <label className="flex items-start gap-3 text-sm text-gray-700">
            ...
            />
            <span>
              <span className="block font-medium">{zh ? '成員資料隱私模式' : 'Member data privacy mode'}</span>
        ...
        <div className="flex items-center justify-end gap-3">
          <Link href={`/${params.locale}/admin/groups`} className="rounded-md border border-gray-200 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">
            {zh ? '取消' : 'Cancel'}
          </Link>
          <button
            type="submit"
