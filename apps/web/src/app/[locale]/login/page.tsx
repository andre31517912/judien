  ...
  const handleLineLogin = async () => {
    ...
    try {
      ...
      if (data.url) {
        ...
        setError(zh ? 'LINE 登入設定有誤。' : 'LINE sign-in is not configured.');
        setLineLoading(false);
      }
    } catch {
      setError(zh ? 'LINE 登入失敗，請稍後再試。' : 'LINE sign-in failed. Please try again.');
      setLineLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto mt-12">
      ...
      >
        ...
        {lineLoading ? (zh ? '連接中…' : 'Connecting…') : (zh ? '使用 LINE 登入' : 'Sign in with LINE')}
      </button>
